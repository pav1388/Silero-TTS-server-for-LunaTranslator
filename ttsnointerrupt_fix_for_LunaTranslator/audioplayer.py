# 'ttsnointerrupt' fix for LunaTranslator win10 x64 v10.15.7.5
# pav13, 26.04.2026
    ...
    def __dotasks(self):
        try:
            while True:
                ...
                if self.current_player and self.current_player.isplaying and not force and globalconfig.get("ttsnointerrupt", False):
                    while self.current_player.isplaying:
                        time.sleep(0.1)
                    if self.playovercallback:
                        self.playovercallback()
                
                self.current_player = playonce(binary, volume)
